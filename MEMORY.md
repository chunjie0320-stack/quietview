...
**⏳ 待办：Vercel 部署（需要梯子）**
- GitHub Pages 国内被墙，需要换成 Vercel
- 需要梯子才能登录 vercel.com 完成 GitHub OAuth
- 等女王大人有梯子时：登录 vercel.com → Settings → Tokens → 创建 token → 给喵子
- 喵子用 Vercel CLI 在沙箱里部署，以后 push 自动更新
- 备选：Gitee Pages（纯国内，不需要梯子）
